...
describe('planned trips', () => {
  it('cree, persiste et passe un trajet a fait avec horodatage', () => {
    const trip = createPlannedTrip(USER_ID, SOURCE, new Date(2026, 6, 16, 8, 30), NOW);
    savePlannedTrip(trip);

    const done = setPlannedTripStatus(USER_ID, trip.id, 'done', new Date(2026, 6, 16, 9, 0));
  ...
  it('upcomingTrips ne retourne que les occurrences a faire, triees par date', () => {
    ...
    savePlannedTrip(later);
    savePlannedTrip(past);
    savePlannedTrip(soon);

    const upcoming = upcomingTrips(loadPlannedTrips(USER_ID), NOW);
    expect(upcoming.map((trip) => trip.id)).toEqual(['soon', 'later']);
  });
});
...
describe('recurring trips', () => {
  it('genere les occurrences aller-retour des jours actifs sur la fenetre, sans doublon au resync', () => {
    ...
    const first = syncRecurringOccurrences(USER_ID, NOW);
    ...
    const second = syncRecurringOccurrences(USER_ID, NOW);
  ...
  it('ne genere rien quand le trajet recurrent est en pause', () => {
    ...
    saveRecurringTrip(template);
    setRecurringTripPaused(USER_ID, template.id, true);

    expect(syncRecurringOccurrences(USER_ID, NOW)).toHaveLength(0);

    setRecurringTripPaused(USER_ID, template.id, false);
    expect(syncRecurringOccurrences(USER_ID, NOW).length).toBeGreaterThan(0);
  });
  ...
  it("une occurrence annulee n'est pas regeneree", () => {
    ...
    const occurrences = syncRecurringOccurrences(USER_ID, NOW);
    const todayId = occurrenceId(template.id, NOW, 'aller');
    expect(occurrences.some((trip) => trip.id === todayId)).toBe(true);

    setPlannedTripStatus(USER_ID, todayId, 'cancelled', NOW);
    const after = syncRecurringOccurrences(USER_ID, NOW);
  ...
  it('supprimer un recurrent retire ses occurrences a faire mais garde les faites', () => {
    ...
    setPlannedTripStatus(USER_ID, doneId, 'done', NOW);

    const { recurring, planned } = deleteRecurringTrip(USER_ID, template.id);
...
describe('summarizeTripActivity', () => {
  it('agrege les trajets faits de la semaine et les compteurs', () => {
    const template = createRecurringTrip(USER_ID, SOURCE, { daysOfWeek: [NOW.getDay()], departureTime: '08:30', returnTime: null }, NOW);
    saveRecurringTrip(template);
    syncRecurringOccurrences(USER_ID, NOW);
    ...
    savePlannedTrip(recentDone);

    const summary = summarizeTripActivity(loadPlannedTrips(USER_ID), [template], NOW);
  ...
  it('borne la semaine au lundi calendaire, pas a 7 jours glissants', () => {
    ...
    savePlannedTrip(sundayDone);

    const summary = summarizeTripActivity(loadPlannedTrips(USER_ID), [], NOW);
