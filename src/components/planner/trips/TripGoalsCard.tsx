...
function GoalActions({ editing, onEdit, onCancel }: { editing: boolean; onEdit: () => void; onCancel: () => void }) {
    if (!editing) {
        return (
            <Button type="button" variant="ghost" size="sm" className="h-6 px-2 text-[11px] text-muted-foreground" onClick={onEdit}>
    ...
    return (
        <span className="flex gap-1">
            ...
            <Button type="button" variant="ghost" size="sm" className="h-6 px-2 text-[11px]" onClick={onCancel}>
                Annuler
            </Button>
        </span>
    );
...
export function TripGoalsCard() {
    ...
    const fields = editing
        ? {
            weeklyTripsGoal: form.register('weeklyTripsGoal', { valueAsNumber: true }),
            weeklySavedGoalGrams: form.register('weeklySavedGoalGrams', { valueAsNumber: true }),
            monthlySavedGoalGrams: form.register('monthlySavedGoalGrams', { valueAsNumber: true }),
        }
        : {};
    ...
    return (
        <form className="rounded-xl border border-border/70 bg-background/75 p-3" aria-label="Objectifs" noValidate onSubmit={commit}>
            <div className="mb-2.5 flex items-center justify-between gap-2">
                ...
                <GoalActions editing={editing} onEdit={() => setEditing(true)} onCancel={cancel} />
            </div>
            <div className="grid gap-3 sm:grid-cols-2 sm:gap-4">
                <div className="grid content-start gap-3">
                    ...
                    <GoalRow
                        ...
                        goal={effectiveGoals.weeklyTripsGoal}
                        unit="trajets"
                        field={fields.weeklyTripsGoal}
                        error={errors.weeklyTripsGoal?.message}
                    />
                    <GoalRow
                        label="CO₂e évité"
                        value={summary.savedThisWeekGrams}
                        goal={effectiveGoals.weeklySavedGoalGrams}
                        unit="g"
                        field={fields.weeklySavedGoalGrams}
                ...
                <div className="grid content-start gap-3 border-t border-border/60 pt-3 sm:border-l sm:border-t-0 sm:pl-4 sm:pt-0">
                    ...
                    <GoalRow label="Trajets faits" value={summary.doneThisMonth} goal={effectiveGoals.weeklyTripsGoal * 4} unit="trajets" />
                    <GoalRow
                        label="CO₂e évité"
                        value={summary.savedThisMonthGrams}
                        goal={effectiveGoals.monthlySavedGoalGrams}
                        unit="g"
                        field={fields.monthlySavedGoalGrams}
