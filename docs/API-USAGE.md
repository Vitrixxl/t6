...
Le corps de `POST /api/transport/journeys` accepte `kind` : `walk`, `bike`, `scooter`, `transit`, `multimodal`. Les onglets du client le transmettent avec les moyens et les types TCL. Absent, il conserve la comparaison de tous les moyens autorisés pour les appelants existants. Multitransport exige une location et un segment TCL dans chaque option ; les deux engins sont recherchés séparément. Réponse 200 : tableau de variantes, éventuellement vide. Réponse 503 : moteur indisponible. OpenAPI est généré depuis `src/contracts/planning.ts`.
