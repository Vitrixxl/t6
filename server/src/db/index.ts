...
// installer, aucun binaire a compiler), habillee par Drizzle.
//
...
// (repositories/) et le driver importe ici. Le schema vit dans schema.ts ;
// les migrations SQL qui en decoulent sont versionnees dans server/drizzle/.
...
/** Base habillee par Drizzle ; `$client` est la connexion bun:sqlite sous-jacente (fermeture). */
export type Db = BunSQLiteDatabase<typeof schema> & { $client: Database };
/** Contexte d'une transaction ouverte : meme API de requete que `Db`. */
export type Tx = Parameters<Parameters<Db['transaction']>[0]>[0];
/** Ce qu'un depot accepte : la base, ou une transaction en cours. */
