...
par `one-to-many`. Un onglet simple utilise un plan ; Multitransport recherche séparément les deux engins en accès et en sortie des TCL. Ce moteur local est la seule source de routage :
...
L’onglet initial est choisi depuis `availableModes` ; tous les onglets restent accessibles pour une dérogation temporaire. PMR utilise le profil fauteuil,
...
sont transmis à MOTIS. Fermer le trajet réinitialise les filtres temporaires depuis le profil.
...
retirées ; l’appel à MOTIS reste interne. La réponse tableau `routeOptions`, vide si aucun trajet ne correspond,
...
La livraison du 6 septembre 2026 utilise l’archive officielle TCL fournie par l’utilisateur (`feed_start_date=20260906`, `feed_end_date=20270104`), versionnée dans `infra/gtfs/tcl.gtfs.zip` et importée dans MOTIS sur 60 jours par `infra/motis-entrypoint.sh` au démarrage du conteneur. Les horaires sont actifs par défaut ; `MOTIS_TRANSIT_ENABLED=false` les désactive dans le moteur et l’API. Le renouvellement automatique et le temps réel restent à intégrer. L’archive ne contient pas `shapes.txt` : les tracés officiels SYTRAL complètent les segments dont la ligne, les quais et leur ordre concordent. Leur distance est mesurée sur ce tracé ; un segment sans correspondance vérifiée reste sans géométrie, avec une estimation de distance et de carbone annoncée. Les accès à pied conservent leur géométrie OSM. Sans archive, le mode `MOTIS_TRANSIT_ENABLED=false` reste disponible avec son bandeau et aucun trajet TCL. Les horaires de recette sont réservés à la CI.
...
Le bouton Départ est disponible dans les cinq onglets. Il permet de choisir une date et une heure locales, ou Maintenant. La validation convertit la saisie en instant ISO selon le fuseau de l’appareil ; departureAt traverse la clé React Query, le contrat partagé, l’API et tous les plans MOTIS. Changer d’onglet conserve cette heure ; fermer le trajet la réinitialise. Planifier reprend la date et l’heure explicitement choisies dans la recherche, y compris l’heure de départ d’une récurrence. Maintenant et un favori planifié sans recalcul conservent une proposition future, pour éviter une comptabilisation immédiate du trajet. Les disponibilités des engins restent celles des flux actuels : elles ne sont pas une prévision à la date choisie.
