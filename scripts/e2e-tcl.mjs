...
import { checkSearchDeparture } from './check-search-departure.mjs';
import { checkSearchFilters } from './check-search-filters.mjs';
...
const search = { kind: 'transit', origin, destination, modes: ['transit'], transitTypes: tb12 ? [3] : [0, 1, 3, 7], accessibilityNeed: false, departureAt };
try {
    ...
    const [walk] = await (await context.request.post(base + '/api/transport/journeys', { data: { ...search, kind: 'walk', modes: [] } })).json();
    ...
    await page.route('**/api/transport/journeys', request => request.continue({
        postData: JSON.stringify({ ...request.request().postDataJSON(), departureAt: request.request().postDataJSON().departureAt ?? departureAt, ...(tb12 ? { transitTypes: [3] } : {}) }),
    }));
    ...
    await checkSearchFilters(page, context, base);
    await checkSearchDeparture(page);
