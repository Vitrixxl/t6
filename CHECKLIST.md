...
- [x] 178 tests verts (19 fichiers), lint 0 erreur, build OK, scénario E2E tutoriel mobile + planification bloquant vert (8/8 assertions), audit axe-core 0 violation WCAG 2.1 A/AA (4 écrans) ; chiffres du dossier extraits automatiquement du build (`output/metrics/`).
...
- Vérification terminal finale: `bun run check` OK (`eslint .`, TypeScript 7 strict, 178 tests, `Bun.build`), `bun run audit:a11y` OK (0 violation sur 4 écrans) et `bun run e2e` OK (8/8).
...
- [x] Vérifications rejouées après bascule : `bun run check` complet (178 tests), E2E 8/8, audit axe-core 0 violation sur 4 écrans.
